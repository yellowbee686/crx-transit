...
console.log('trail: ', _trail);          
  ...
  // 从fileNames构造出files
  function makeFiles(fileNames){
    var files = [];
    var path = 'D:\\crx-transit\\data\\';
    for(var i=0;i<fileNames.length;i++){
      files.push(new File([""], path+fileNames[i]))
    }
    return files;
  };
  ...
  module.exports = function load(files) {
      ...
      Array.prototype.forEach.call(files, function(f) {
        ...
        //这里导致1种后缀只加载了最后一个文件，但后面也是这样用的...
        //resources.push(resources[ext] = parse_mdict(f, ext));
        resources.push(parse_mdict(f, ext));
      });
      //  Promise.all 遍历执行迭代器中的每一项，任何一项不满足都会直接返回
